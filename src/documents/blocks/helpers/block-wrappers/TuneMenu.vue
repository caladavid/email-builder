  ...
  for (const [id, b] of Object.entries(nDocument)) {
    ...
    switch (block.type) {
      ...
      case 'ColumnsContainer':
        nDocument[id] = { type: 'ColumnsContainer', data: { style: block.data.style, props: { ...block.data.props, columns: block.data.props?.columns?.map((c) => ({ childrenIds: filterChildrenIds(c.childrenIds) })) } } as ColumnsContainerProps }
        break
      default:
        nDocument[id] = block
    }
...
function handleMoveClick(direction: 'up' | 'down') {
  ...
  for (const [id, b] of Object.entries(nDocument)) {
    ...
    switch (block.type) {
      case 'EmailLayout':
        nDocument[id] = { ...block, data: { ...block.data, childrenIds: moveChildrenIds(block.data.childrenIds) } }
        break
      case 'Container':
        nDocument[id] = { ...block, data: { ...block.data, props: { ...block.data.props, childrenIds: moveChildrenIds(block.data.props?.childrenIds) } } }
        break
      case 'ColumnsContainer':
        nDocument[id] = { type: 'ColumnsContainer', data: { style: block.data.style, props: { ...block.data.props, columns: block.data.props?.columns?.map((c) => ({ childrenIds: moveChildrenIds(c.childrenIds) })) } } as ColumnsContainerProps }
        break
      default:
        nDocument[id] = block
    }
  ...
  for (const [blockId, block] of Object.entries(newDocument)) {
    if (block.type === 'EmailLayout' && block.data.childrenIds) {
      ...
      if (index !== -1) {
        ...
        newDocument[blockId] = { ...block, data: { ...block.data, childrenIds: newChildrenIds } };
    ...
    if (block.type === 'Container' && block.data.props?.childrenIds) {
      ...
      if (index !== -1) {
        ...
        newChildrenIds.splice(index + 1, 0, newId);
        newDocument[blockId] = { ...block, data: { ...block.data, props: { ...block.data.props, childrenIds: newChildrenIds } } };
        inspectorDrawer.setDocument(newDocument);
        return;
      }
    }
    if (block.type === 'ColumnsContainer' && block.data.props?.columns) {
      for (let colIndex = 0; colIndex < block.data.props.columns.length; colIndex++) {
        ...
        if (index !== -1) {
          ...
          newDocument[blockId] = { ...block, data: { ...block.data, props: { ...block.data.props, columns: newColumns } } };
          inspectorDrawer.setDocument(newDocument);
