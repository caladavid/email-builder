...
/*     private processElement(
    ...
    private processElement(
        ...
        if (tagName === "table") {  
            console.log("🔍 Analizando tabla:", element);  
            console.log("isInlineCompactTable:", this.isInlineCompactTable(element));  
            console.log("isColumnsTable:", this.isColumnsTable(element));  
        ...
        // Detectar tablas compactas con iconos  
        if (tagName === "table" && this.isInlineCompactTable(element)) {  
            const id = this.createInlineCompactTableBlock(element, currentStyles);  
            if (id) return id;  
        }
        ...
        if (isTextTag) {
            ...
            // Esto arregla el caso <td>(12px) -> <p>(sin estilo)
    ...
    private processInlineContent(
        ...
        Array.from(element.childNodes).forEach((node) => {
            ...
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                // Limpieza básica (sin lógica de viñetas ni saltos extra)
                ...
                if (isBullet) {
                    hasListStructure = true; // 👈 3. Activamos la bandera si encontramos una lista
                    
                    if (text.length > 0 && !text.endsWith('\n')) {
                        // ... (Tu lógica de insertar \n) ...
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (tag === 'ul' || tag === 'ol' || tag === 'li') {
                    hasListStructure = true; // 👈 4. Refuerzo por etiqueta
                }
...
/*                 if (tag === "a") {  
                    ...
                    // SOLO convertir a markdown para iconos en tablas compactas  
                    if (img && href && this.isInsideInlineCompactTable(el)) {  
                        const imgSrc = img.getAttribute("src") || "";  
                        const imgAlt = img.getAttribute("alt") || "";  
                        append(`[![${imgAlt}](${imgSrc})](${href})`);  
                        return;  
                    }  
                    
                    // Enlaces de texto normales (sin cambios)  
                    const linkText = el.textContent?.trim() || "";  
                    if (href && linkText) {  
                        append(`[${linkText}](${href})`);  
                        return;  
                ...
                if (childRes.text.length) {
                    ...
                    // Aplicar formato del nodo actual (Lógica que te funcionaba bien)
                    ...
                    // Si el elemento tiene una fuente definida y no es la default, la guardamos
                    ...
                    if (childStyles.fontSize) {
                        ...
                        // Si el tamaño no es el default (16px), lo guardamos
                    ...
                    // Solo agregamos si hay formato real
                    if (hasFormat) {
                        formats.push(fmt);
                    }
                }
...
/*     private processChildren(parentElement: Element, targetArray: string[], inheritedStyles: Record<string, string>): void {
    ...
    private processChildren(parentElement: Element, targetArray: string[], inheritedStyles: Record<string, string>): void {
        // GrapesJS recorre childNodes, no children. Esto es vital para texto mixto.
        Array.from(parentElement.childNodes).forEach((node) => {
            const id = this.processNode(node, inheritedStyles); // Delegamos a processNode
            if (id) targetArray.push(id);
        });
    }
    ...
    private processNode(node: Node, inheritedStyles: Record<string, any>): string | null {
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            ...
            // Ignoramos saltos de línea vacíos que son solo formateo del código HTML
            if (!textContent || !textContent.trim()) return null;
            
            // Creamos un bloque de texto puro
            return this.createTextBlock(textContent, [], inheritedStyles);
        }

        // 2. Elementos HTML (Tags)
        if (node.nodeType === Node.ELEMENT_NODE) {
...
private isColumnsTable(element: Element): boolean {  
    ...
    private isInlineCompactTable(table: Element): boolean {  
        ...
        // CAMBIO: Permitir cualquier número de filas, pero buscar al menos una con iconos  
        for (const row of rows) {  
            const tds = Array.from(row.querySelectorAll(":scope > td"));  
            
            // Permitir 1 celda con múltiples imágenes  
            const hasMultipleImgsInSingleCell = tds.length === 1 &&   
                tds[0].querySelectorAll("img").length >= 2;  
            ...
            if (simple >= Math.max(2, Math.floor(tds.length * 0.8))) {  
                return true; // Encontramos al menos una fila válida  
            }  
        }  
    ...
    private isSimpleInlineCell(td: Element): boolean {  
        ...
        // NUEVO: Detectar múltiples imágenes en la misma celda  
    ...
    // Crea contenedor Flex (para 4 iconos, etc)
    private createFlexContainerFromIds(childrenIds: string[], styles: any): string {
        const id = uuidv4();
        const count = childrenIds.length;
        
        // Si hay múltiples hijos en una fila (TR), distribuimos el ancho
        if (count > 1) {
            ...
            childrenIds.forEach(childId => {
                if (this.blocks[childId]?.data?.style) {
                    ...
                    s.maxWidth = "100%";
                    // Centrar contenido interno (como imágenes)
            ...
            for (const cell of cells) {
                ...
                const contentId = this.processElement(cellElement, stylesForChildren);
                 ...
                 const imagesInCell = cellElement.querySelectorAll('img');  
                    console.log(`🖼️ Celda con ${imagesInCell.length} imágenes:`, cellElement);  
                    
                    if (imagesInCell.length > 0) {  
                        imagesInCell.forEach((img, i) => {  
                            console.log(`  - Imagen ${i}:`, img.getAttribute('src'), img);  
                        });  
                    }
...
/*             if (producedColumns.length > 3) {  
                const childIds = producedColumns.map(col => col.childrenIds[0]);  
                const containerId = this.createContainerBlock(childIds, {  
                    ...styles,  
                    display: 'flex',  
                    flexWrap: 'wrap',  
                    gap: '0px',  
    ...
    private createTextBlock(text: string, formats: any[], styles: any, forceMarkdown: boolean = false): string | null {        
        ...
        // 🔥 NUEVO: Si tiene saltos de línea (\n), también es "Markdown" visualmente
...
/*         const defaultStyles = {
    ...
    private extractStyles(element: Element, inheritedStyles: Record<string, any> = {}): any {  
        ...
        if (widthAttr) {
            // Si es píxel o porcentaje, lo pasamos. GrapesJS suele respetar esto.
...
/*     private normalizeColor(color: string): string {
    ...
    private isInsideInlineCompactTable(element: Element): boolean {  
        let parent = element.parentElement;  
        while (parent) {  
            if (parent.tagName.toLowerCase() === 'table' && this.isInlineCompactTable(parent)) {  
                return true;  
            }  
...
private buildConfiguration(): TEditorConfiguration {
        const rootId = "root";
        const hexRegex = /^#[0-9a-fA-F]{6}$/i; // Regex estricto para HEX

        Object.keys(this.blocks).forEach(blockId => {
            const block = this.blocks[blockId];
            if (block.data && block.data?.style) {

                // Validar Paddings numéricos
                ...
                // 🔥 FIX COLOR REGEX 🔥
                // Si el backgroundColor no cumple el regex Hex, elimínalo.
                if (block.data.style.backgroundColor) {
                    const normalized = this.normalizeColor(block.data.style.backgroundColor);
                    if (hexRegex.test(normalized)) {
                        block.data.style.backgroundColor = normalized;
                    } else {
                        delete block.data.style.backgroundColor;
                    }
                }
                
                // Limpiar color de texto si no es válido
                if (block.data.style.color && !hexRegex.test(block.data.style.color)) {
