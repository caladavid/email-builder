...
import { DataTableMatcher } from "./matchers/DataTableMatcher";
import { GreedyTextMatcher } from "./matchers/GreedyTextMatcher";
import { LayoutMatcher } from "./matchers/LayoutMatcher";
import { GridSystemMatcher } from "./matchers/GridSystemMatcher";
import { TableToTextMatcher } from "./matchers/TableToTextMatcher";
import { StructuralTableMatcher } from "./matchers/StructuralTableMatcher";
import { TableBlockMatcher } from "./matchers/TableBlockMatcher";

export class HTMLToBlockParser {

    constructor(private mode: 'EMAIL' | 'WEB' = 'EMAIL') {}
    ...
    private async parseHtmlToBlocksWithErrors(htmlContent: string): Promise<{
        ...
        try {
            ...
            if (embeddedCss.trim().length > 0) {
                // A. Procesamos Media Queries para el soporte móvil (Fase 3)
                // Esto llena el mapa this.mobileStylesMap
                try {
                ...
                // B. Inicializamos el CSS Parser Global con estos estilos
                // Esto es CRÍTICO para que clases como .u-row o .u-col sean reconocidas
                try {
                    // Nota: Si ya existía un CSSParser (de archivos externos), aquí lo estamos 
                    // sobrescribiendo para priorizar los estilos del documento actual, 
                    // que es lo habitual en templates de email únicos.
                    this.cssParser = new CSSParser(embeddedCss);
                } catch (e) {
                    warnings.push(new ParseError(
                        'Error procesando estilos incrustados en el HTML',
                        'CSS_ERROR',
                        { originalError: e }
                    ));
                }
            ...
            // Clean up the structure by merging redundant nested containers
    ...
    private processElement(
        ...
        /* if (tagName === 'img') {
            console.warn('⚠️ [SISTEMA ANTIGUO] La imagen pasó al fallback (El matcher falló):', element);
        } else {
            console.log('ℹ️ [SISTEMA ANTIGUO] Procesando elemento no migrado:', tagName);
        } */
