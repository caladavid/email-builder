...
import { formatTextToMarkdown } from "./markdown-formatter";
    ...
    Object.keys(processedDoc).forEach(blockId => {
        ...
        if (block.data?.props?.text) {
            ...
            if (block.type === 'Text' && block.data.props.formats && block.data.props.formats.length > 0) {
                processedText = formatTextToMarkdown(processedText, block.data.props.formats);
                block.data.props.markdown = true;
            }

            processedText = processDocumentVariables(processedText, globalVariables || {});
